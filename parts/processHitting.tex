...
A  \emph{plural action} is an action noted by: $h=\PHfrappe{A}{b_j}{b_k}$ with $A \in \PHl^{\diamond} \wedge b_j\neq b_k \wedge if b_i \in A \Rightarrow A=b_j$.  We note that $\PHl^{\diamond}$ is the set of all the sub-states of $\PHl$.
\end{definition}
...
\label{def:TimedAction}
A timed plural action is a plural action with a delay $D$: $\PHfrappedelay{A}{D}{b_i}{b_j}$ where $D \in \mathds{R}^+$, $A \in \PHl^{\diamond}$, and $b_i$, $b_j$ where  $b_i \not = b_j$ are two processes of the target sort $b$. If $b_i \in A$, $A=b_i$.
\end{definition}
...
\label{def:playableAction}
Let $\PH = (\PHs,\PHl,\PHap)$ be a Process Hitting with timed plural actions and $s \in \PHl$ be the state of $\PH$ at time $t$ .
