...
We note that during these last years the Process Hitting framework was improved and we added new type of sorts like cooperative sorts and new actions like plural actions \cite{folschette-phd-14}, actions with priority and actions with delay.
...
Note that if all actions delays is set to 1 it is equivalent to Process Hitting without delays.
The conditions of use of these new actions are as follows.

\begin{definition} [Playable timed plural action]
\label{def:playableAction}
Let $\PH = (\PHs,\PHl,\PHap)$ be a Process Hitting with timed plural actions and $s \in \PHl$ a state of $PH$.
...
The PH framework was chosen for several reasons.
